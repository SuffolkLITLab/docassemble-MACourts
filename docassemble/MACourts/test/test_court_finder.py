...
class TestCourtFinder(unittest.TestCase):
    def setUp(self):
        data_path = data_path = (
            Path(__file__).resolve().parent.joinpath("../data/sources")
        )
    ...
    def test_search_boston(self):
        address = Address(
            ...
            norm=Address(
                address="1234 Soliders Field Road",
                city="Boston",
                county="Suffolk Courty",
                state="Massachusetts",
                zip="02135",
            ),
        )
        court_list = self.all_courts.matching_courts(
            address, court_types=self.court_types
        )
    ...
    def test_find_sjc(self):
        ...
        court_list = self.all_courts.matching_courts(
            address, court_types=["Supreme Judicial Court"]
        )
    ...
    def test_random_points(self):
        ...
        for street, city, county, zip, latitude, longitude in (
            ...
            (
                "10 Catamount Hill Road Number One",
                "Colrain",
                "Franklin County",
                "01340",
                42.674503697774504,
            ...
            address = Address(
                address=street, city=city, county=county, state="Massachusetts", zip=zip
            )
    ...
    @given(
        ...
        city=st.text(),
        county=st.text(),
        zip=st.from_regex(r"[0-9]{5}"),
    )
    def test_search_all(self, address, city, county, zip):
        address = Address(
            address=address, city=city, county=county, state="Massachusetts", zip=zip
        )
        address.norm_long = address
        address.norm = address
