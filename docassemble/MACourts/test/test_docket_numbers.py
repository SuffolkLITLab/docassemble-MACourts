...
class TestDocketNumbers(unittest.TestCase):
    ...
    def _check_court_name(self, docket_number, expected_name):
        self.assertEqual(
            self.all_courts.court_from_docket_number(docket_number).name, expected_name
        )
    ...
    def _expect_court_not_found(self, docket_number):
        try:
            ...
            self.fail(
                f"{docket_number} shouldn't be a valid number, but it found {[str(c) for c in courts]}"
            )
        except KeyError:
            # All good
            pass
    ...
    def test_parse_docket_numbers(self):
        ...
        self._check_court_name(
            "1401CV001026", "Central Division, Boston Municipal Court"
        )
